...
public sealed class TransferSession
{
    ...
    private TransferSession(byte[] data, string fileName, string mimeType, int chunkSize, QrLayoutMode layoutMode,
        int quadGapPercent)
    {
        Data = data;
    ...
    public static TransferSession Create(byte[] data, string fileName, string mimeType, int chunkSize,
        QrLayoutMode layoutMode = QrLayoutMode.Single, int quadGapPercent = 4)
    {
        ...
        {
            throw new ArgumentOutOfRangeException(nameof(chunkSize), "V1.7.2 分片大小必须在 100～2800 字节之间。");
        }

        if (layoutMode is not (QrLayoutMode.Single or QrLayoutMode.Quad))
        {
            throw new ArgumentOutOfRangeException(nameof(layoutMode));
        }

        if (quadGapPercent is < 2 or > 20)
        {
            throw new ArgumentOutOfRangeException(nameof(quadGapPercent), "四码间距必须在2%～20%之间。");
        }

        return new TransferSession(data, fileName, mimeType, chunkSize, layoutMode, quadGapPercent);
    }
    ...
    private string CreateDataFrame(int index, int round)
    {
        ...
        var seed = GetWhiteningSeedForRound(round);
    ...
    public static byte GetWhiteningSeedForRound(int round)
    {
        if (round < 1) throw new ArgumentOutOfRangeException(nameof(round));
        return (byte)(((round - 1) % 255) + 1);
    }

(() => {
  ...
  document.documentElement.dataset.qtxVersion = "1.8.4";
  ...
  const FAILURE_SAMPLE_LIMIT = 12;
  const EXPOSURE_SAMPLE_LIMIT = 512;
  ...
  const ui = {
    ...
    distributionToggleLabel: document.getElementById("distributionToggleLabel"),
    distributionDetails: document.getElementById("distributionDetails"),
    distributionSamples: document.getElementById("distributionSamples"),
    distributionAverage: document.getElementById("distributionAverage"),
    distributionP50: document.getElementById("distributionP50"),
    distributionP95: document.getElementById("distributionP95"),
    distributionP99: document.getElementById("distributionP99"),
    distributionMax: document.getElementById("distributionMax"),
    zxingErrorsToggle: document.getElementById("zxingErrorsToggle"),
    zxingErrorsToggleLabel: document.getElementById("zxingErrorsToggleLabel"),
    zxingErrorsDetails: document.getElementById("zxingErrorsDetails"),
    zxingSuccess: document.getElementById("zxingSuccess"),
    zxingNoSymbol: document.getElementById("zxingNoSymbol"),
    zxingFormat: document.getElementById("zxingFormat"),
    zxingChecksum: document.getElementById("zxingChecksum"),
    zxingUnsupported: document.getElementById("zxingUnsupported"),
    zxingException: document.getElementById("zxingException"),
    zxingErrorsNote: document.getElementById("zxingErrorsNote"),
    qualityToggle: document.getElementById("qualityToggle"),
    qualityToggleLabel: document.getElementById("qualityToggleLabel"),
    qualityDetails: document.getElementById("qualityDetails"),
    qualitySamples: document.getElementById("qualitySamples"),
    qualityBrightness: document.getElementById("qualityBrightness"),
    qualityContrast: document.getElementById("qualityContrast"),
    qualitySharpness: document.getElementById("qualitySharpness"),
    qualityBlackClip: document.getElementById("qualityBlackClip"),
    qualityWhiteClip: document.getElementById("qualityWhiteClip"),
    failureSamplesToggle: document.getElementById("failureSamplesToggle"),
    failureSamplesToggleLabel: document.getElementById("failureSamplesToggleLabel"),
    failureSamplesDetails: document.getElementById("failureSamplesDetails"),
    failureSamplesCount: document.getElementById("failureSamplesCount"),
    failureGallery: document.getElementById("failureGallery"),
    clearFailureSamples: document.getElementById("clearFailureSamplesButton"),
    exposureToggle: document.getElementById("exposureToggle"),
    exposureToggleLabel: document.getElementById("exposureToggleLabel"),
    exposureDetails: document.getElementById("exposureDetails"),
    exposureSamples: document.getElementById("exposureSamples"),
    exposureShutter: document.getElementById("exposureShutter"),
    exposureCurrent: document.getElementById("exposureCurrent"),
    exposureIso: document.getElementById("exposureIso"),
    exposureFrameDuration: document.getElementById("exposureFrameDuration"),
    exposureFps: document.getElementById("exposureFps"),
    exposureRatio: document.getElementById("exposureRatio"),
    exposureAverage: document.getElementById("exposureAverage"),
    exposureP50: document.getElementById("exposureP50"),
    exposureP95: document.getElementById("exposureP95"),
    exposureMin: document.getElementById("exposureMin"),
    exposureMax: document.getElementById("exposureMax")
  };
  ...
  let durationDistribution = [];
  let zxingErrorStats = createZxingErrorStats();
  let qualityStats = createQualityStats();
  let failureSamples = [];
  let exposureSamples = [];
  let lastDiagnosticRenderAt = 0;
  let browserDiagnosticFrame = 0;
  let lastBrowserFailureSampleAt = 0;
  ...
  let activeChunkBatch = null;
  let finalizeAfterChunkBatch = false;
  ...
  ui.distributionToggle.addEventListener("change", toggleDurationDistribution);
  ui.zxingErrorsToggle.addEventListener("change", toggleZxingErrors);
  ui.qualityToggle.addEventListener("change", toggleImageQuality);
  ui.failureSamplesToggle.addEventListener("change", toggleFailureSampling);
  ui.exposureToggle.addEventListener("change", toggleExposureDiagnostics);
  ui.clearFailureSamples.addEventListener("click", clearFailureSamples);
  ...
  window.onNativeQuadScanSample = handleNativeQuadScanSample;
  ...
  async function startNativeCamera(requestedDeviceId = "") {
    try {
      ...
      setMessage("V1.8.4 正在直接读取相机 Y Plane；兼容QTX1 V1单码与V2单码/四码。", "info");
  ...
  function acknowledgeNativeResult() {
    if (window.AndroidBridge && typeof window.AndroidBridge.acknowledgeNativeResult === "function") {
      try { window.AndroidBridge.acknowledgeNativeResult(); } catch {}
    }
  }

  async function handleNativeScanSample(raw, sizeRatio, metrics) {
    try {
      await handleNativeScanSampleCore(raw, sizeRatio, metrics);
    } finally {
      acknowledgeNativeResult();
    }
  }

  async function handleNativeScanSampleCore(raw, sizeRatio, metrics) {
    if (!nativePipelineActive || !scanning) return;
    if (durationDistributionEnabled && Number.isFinite(metrics?.total)) recordDurationDistribution(metrics.total);
    if (zxingErrorsEnabled && typeof metrics?.decodeStatus === "string") recordZxingStatus(metrics.decodeStatus);
    if (imageQualityEnabled && metrics?.qualitySample) recordQualitySample(metrics);
    if (exposureDiagnosticsEnabled && metrics?.exposureSample) recordExposureSample(metrics);
    if (failureSamplingEnabled && metrics?.failureImage) {
      addFailureSample(`data:image/jpeg;base64,${metrics.failureImage}`, metrics.frameNumber || 0, metrics.decodeStatus || "no_symbol");
    }
    if (nativeResultBusy) return;
    ...
    try {
      ...
      if (timing && performanceTimingEnabled) recordTimingSample(timing);
      nativeResultBusy = false;
    }
  }

  async function handleNativeQuadScanSample(rawValues, metrics) {
    try {
      await handleNativeQuadScanSampleCore(rawValues, metrics);
    } finally {
      acknowledgeNativeResult();
    }
  }

  async function handleNativeQuadScanSampleCore(rawValues, metrics) {
    if (!nativePipelineActive || !scanning || !Array.isArray(rawValues)) return;
    const statuses = Array.isArray(metrics?.decodeStatuses) ? metrics.decodeStatuses : [];
    if (durationDistributionEnabled && Number.isFinite(metrics?.total)) recordDurationDistribution(metrics.total);
    if (zxingErrorsEnabled) {
      for (const status of statuses) recordZxingStatus(typeof status === "string" ? status : "exception");
    }
    if (imageQualityEnabled && metrics?.qualitySample) recordQualitySample(metrics);
    if (exposureDiagnosticsEnabled && metrics?.exposureSample) recordExposureSample(metrics);
    if (failureSamplingEnabled && metrics?.failureImage) {
      const reason = statuses.find(status => status && status !== "success") || "no_symbol";
      addFailureSample(`data:image/jpeg;base64,${metrics.failureImage}`, metrics.frameNumber || 0, `quad:${reason}`);
    }
    if (nativeResultBusy) return;
    ...
    if (timing) {
      ...
      timing.nativeCombined = finiteDuration(metrics?.decoder);
      timing.total = finiteDuration(metrics?.total);
      timing.detected = rawValues.some(Boolean);
      timing.relocated = Boolean(metrics?.fullLocate);
      timing.qrDetect = finiteDuration(metrics?.decoder);
      failureRelocateCount = Number.isFinite(metrics?.failureRelocateCount) ? metrics.failureRelocateCount : failureRelocateCount;
    }

    const uniqueRawValues = [...new Set(rawValues.filter(raw => typeof raw === "string" && raw.length >= HEADER_LENGTH))];
    const now = performance.now();
    if (uniqueRawValues.length) {
      lastDetectedAt = now;
      updateDistanceGuide(Number.isFinite(metrics?.sizeRatio) ? metrics.sizeRatio : 0.42);
    } else if (now - lastDetectedAt > 1200) {
      updateDistanceGuide(null);
    }
    ...
    try {
      const protocolStarted = timing ? performance.now() : 0;
      const pageBatch = [];
      activeChunkBatch = pageBatch;
      finalizeAfterChunkBatch = false;
      await Promise.all(uniqueRawValues.map(raw => {
          lastRaw = raw;
          lastRawAt = now;
          return processFrame(raw);
        }));
      const shouldFinalize = finalizeAfterChunkBatch;
      activeChunkBatch = null;
      finalizeAfterChunkBatch = false;
      await storeChunkBatch(pageBatch);
      if (shouldFinalize && active && received.size === active.meta.t) await finalizeTransfer();
      if (timing) timing.protocol += performance.now() - protocolStarted;
    } finally {
      activeChunkBatch = null;
      finalizeAfterChunkBatch = false;
      if (timing) {
        timing.total += timing.protocol;
        recordTimingSample(timing);
      }
      nativeResultBusy = false;
    }
  }
  ...
  function configureNativeDiagnostics() {
    if (!nativeScannerSupported()) return;
    try {
      window.AndroidBridge.configureNativeScanner(
        effectiveRelocationMode(),
        relocationInterval,
        performanceTimingEnabled || durationDistributionEnabled,
        zxingErrorsEnabled,
        imageQualityEnabled,
        failureSamplingEnabled,
        exposureDiagnosticsEnabled
      );
    } catch (error) {
      console.warn("Unable to configure native diagnostics", error);
    }
  }

  function applyQrLayout(layout, announce = true) {
    const nextLayout = layout === 4 ? 4 : 1;
    const changed = activeQrLayout !== nextLayout;
    activeQrLayout = nextLayout;
    ui.layoutStatus.textContent = nextLayout === 4 ? "田字形四码 · 四线程" : "单二维码";
    document.documentElement.dataset.qtxLayout = String(nextLayout);
    if (nativeScannerSupported() && typeof window.AndroidBridge.configureNativeLayout === "function") {
      try { window.AndroidBridge.configureNativeLayout(nextLayout); }
      catch (error) { console.warn("Unable to configure QR layout", error); }
    }
    if (changed) resetQrTracking();
    if (announce && changed) {
      setMessage(nextLayout === 4
        ? "元数据声明为田字形四码，已切换为四ROI并行解码。"
        : "元数据声明为单二维码，已切换为单任务解码。", "success");
    }
  }
  ...
  function toggleDeveloperMode() {
    ...
    if (!developerModeEnabled) {
      disableAllDeveloperDiagnostics();
    }
    lastRaw = "";
    resetQrTracking();
  ...
  function disableAllDeveloperDiagnostics() {
    ui.timingToggle.checked = false;
    ui.distributionToggle.checked = false;
    ui.zxingErrorsToggle.checked = false;
    ui.qualityToggle.checked = false;
    ui.failureSamplesToggle.checked = false;
    ui.exposureToggle.checked = false;
    performanceTimingEnabled = false;
    durationDistributionEnabled = false;
    zxingErrorsEnabled = false;
    imageQualityEnabled = false;
    failureSamplingEnabled = false;
    exposureDiagnosticsEnabled = false;
    activeTimingSample = null;
    ui.timingDetails.hidden = true;
    ui.timingOffNote.hidden = false;
    ui.timingToggleLabel.textContent = "关闭";
    renderDiagnosticControls();
    resetDeveloperDiagnosticData();
    configureNativeDiagnostics();
  }
  ...
  function startBrowserScanLoop() {
    const generation = ++scanLoopGeneration;
    requestAnimationFrame(now => scanLoop(now, generation));
  }

  async function scanLoop(now, generation) {
    if (!scanning || generation !== scanLoopGeneration) return;
    requestAnimationFrame(nextNow => scanLoop(nextNow, generation));
    ...
    const timing = performanceTimingEnabled || durationDistributionEnabled ? createTimingSample(engine) : null;
    const totalStarted = timing ? performance.now() : 0;
    activeTimingSample = performanceTimingEnabled ? timing : null;
    try {
      const detectorStarted = timing ? performance.now() : 0;
      const detailedTiming = performanceTimingEnabled ? timing : null;
      const detection = engine === "native" ? await decodeNative(detailedTiming) : decodeWithJsQr(detailedTiming);
      if (!scanning || generation !== scanLoopGeneration) return;
      ...
      collectBrowserDiagnostics(Boolean(detection), engine, now);
      if (detection) {
        lastDetectedAt = now;
      ...
      activeTimingSample = null;
      if (timing && performanceTimingEnabled) recordTimingSample(timing);
      if (timing && durationDistributionEnabled) recordDurationDistribution(timing.total);
      decoding = false;
    }
  }
  ...
  function collectBrowserDiagnostics(detected, engine, now) {
    browserDiagnosticFrame++;
    const qualityDue = imageQualityEnabled && browserDiagnosticFrame % 15 === 0;
    const failureDue = failureSamplingEnabled && !detected && now - lastBrowserFailureSampleAt >= 350;
    if (!qualityDue && !failureDue) return;
    ...
    if (qualityDue) {
      ...
      recordQualitySample(measureRgbaQuality(image.data, width, height));
    }
    if (failureDue) {
      lastBrowserFailureSampleAt = now;
      addFailureSample(ui.canvas.toDataURL("image/jpeg", 0.62), browserDiagnosticFrame, `no_symbol:${engine}`);
    }
  }

  function measureRgbaQuality(data, width, height) {
    let count = 0;
    let sum = 0;
    let sumSquares = 0;
    let black = 0;
    ...
    for (let y = step; y < height - step; y += step) {
      for (let x = step; x < width - step; x += step) {
        ...
        if (value < 20) black++;
        if (value > 235) white++;
        const laplacian = 4 * value - luminanceAt(x - step, y) - luminanceAt(x + step, y) - luminanceAt(x, y - step) - luminanceAt(x, y + step);
    ...
    return {
      ...
      sharpness: laplacianCount ? Math.sqrt(laplacianSquares / laplacianCount) : 0,
      blackClip: count ? black * 100 / count : 0,
      whiteClip: count ? white * 100 / count : 0
    };
  }
  ...
  async function handleStartFrame(frame) {
    let metadata;
    try {
      metadata = normalizeMetadata(JSON.parse(new TextDecoder().decode(frame.payload)));
    ...
    applyQrLayout(metadata.q, !active);
    ...
    if (!active) {
      ...
      try {
        ...
        console.warn("Unable to remove stale transfer chunks", error);
      }
  ...
  async function handleDataFrame(frame) {
    ...
    if (received.size === active.meta.t) {
      if (activeChunkBatch) finalizeAfterChunkBatch = true;
      else await finalizeTransfer();
    }
  }
  ...
  function validateMetadata(meta, total) {
    if (!meta || (meta.v !== 1 && meta.v !== 2)) throw new Error("只支持QTX1元数据协议V1或V2");
    if (meta.q !== 1 && meta.q !== 4) throw new Error("二维码布局参数不合法");
    if (!Number.isSafeInteger(meta.g) || meta.g < 2 || meta.g > 20) throw new Error("四码间距参数不合法");
  ...
  async function finalizeTransfer() {
    ...
    try {
      ...
      const nativeShareAvailable = Boolean(window.AndroidBridge
        && typeof window.AndroidBridge.beginShare === "function"
        && typeof window.AndroidBridge.finishShare === "function");
      const webShareAvailable = Boolean(navigator.share && navigator.canShare && navigator.canShare({ files: [completedFile] }));
      ui.share.hidden = !(nativeShareAvailable || webShareAvailable);
      updateProgress();
      stopCamera();
      setStatus("接收成功", "success");
  ...
  async function shareCompletedFile() {
    if (!completedFile || !completedBlob) return;
    if (window.AndroidBridge
      && typeof window.AndroidBridge.beginShare === "function"
      && typeof window.AndroidBridge.finishShare === "function") {
      await shareThroughAndroidAndDelete();
      return;
    }
    ...
    try {
      ...
      await resetTransfer();
      setMessage("文件已转发，当前接收任务及文件数据已删除。", "success");
    } catch (error) {
      if (error.name !== "AbortError") setMessage(`分享失败：${error.message}`, "error");
    }
  }

  async function shareThroughAndroidAndDelete() {
    ui.share.disabled = true;
    ui.share.textContent = "正在准备转发…";
    try {
      const bytes = new Uint8Array(await completedBlob.arrayBuffer());
      const mimeType = completedBlob.type || "application/octet-stream";
      if (!window.AndroidBridge.beginShare(completedFile.name, mimeType, bytes.length)) {
        throw new Error("无法创建转发临时文件");
      }
      ...
      for (let offset = 0; offset < bytes.length; offset += blockSize) {
        const block = bytes.subarray(offset, Math.min(bytes.length, offset + blockSize));
        if (!window.AndroidBridge.appendSave(bytesToBase64(block))) {
          throw new Error("写入转发临时文件失败");
        }
        if (offset > 0 && offset % (blockSize * 16) === 0) await delay(0);
      }

      if (!window.AndroidBridge.finishShare()) throw new Error("转发临时文件不完整");
      await resetTransfer();
      setMessage("已打开系统分享面板，当前接收任务及文件数据已删除；分享临时副本将在10分钟后清理。", "success");
    } catch (error) {
      if (window.AndroidBridge.cancelSave) window.AndroidBridge.cancelSave();
      setMessage(`转发失败：${error.message}`, "error");
    } finally {
      ui.share.disabled = false;
      ui.share.textContent = "转发并删除";
    }
  }
  ...
  function showFileMetadata() {
    ...
    const layout = active.meta.q === 4 ? "田字形四码" : "单二维码";
    ui.fileMeta.textContent = `${formatBytes(active.meta.s)} · ${active.meta.t} 个数据帧 · ${layout} · 会话 ${active.id}`;
  }
  ...
  function createZxingErrorStats() {
    return { success: 0, no_symbol: 0, format: 0, checksum: 0, unsupported: 0, exception: 0 };
  }

  function createQualityStats() {
    return { count: 0, brightness: 0, contrast: 0, sharpness: 0, blackClip: 0, whiteClip: 0 };
  }

  function toggleDurationDistribution() {
    durationDistributionEnabled = developerModeEnabled && ui.distributionToggle.checked;
    durationDistribution = [];
    ui.distributionDetails.hidden = !durationDistributionEnabled;
    ui.distributionToggleLabel.textContent = durationDistributionEnabled ? "开启" : "关闭";
    renderDurationDistribution();
    configureNativeDiagnostics();
  }

  function toggleZxingErrors() {
    zxingErrorsEnabled = developerModeEnabled && ui.zxingErrorsToggle.checked;
    zxingErrorStats = createZxingErrorStats();
    ui.zxingErrorsDetails.hidden = !zxingErrorsEnabled;
    ui.zxingErrorsToggleLabel.textContent = zxingErrorsEnabled ? "开启" : "关闭";
    ui.zxingErrorsNote.textContent = nativeScannerSupported()
      ? "ReturnErrors 已用于区分 ZXing-C++ 内部失败；关闭开关后恢复快速模式。"
      : "当前不是原生 APK，无法获得 ZXing-C++ 内部错误。";
    renderZxingErrors();
    configureNativeDiagnostics();
  }

  function toggleImageQuality() {
    imageQualityEnabled = developerModeEnabled && ui.qualityToggle.checked;
    qualityStats = createQualityStats();
    ui.qualityDetails.hidden = !imageQualityEnabled;
    ui.qualityToggleLabel.textContent = imageQualityEnabled ? "开启" : "关闭";
    browserDiagnosticFrame = 0;
    renderQualityStats();
    configureNativeDiagnostics();
  }

  function toggleFailureSampling() {
    failureSamplingEnabled = developerModeEnabled && ui.failureSamplesToggle.checked;
    ui.failureSamplesDetails.hidden = !failureSamplingEnabled;
    ui.failureSamplesToggleLabel.textContent = failureSamplingEnabled ? "开启" : "关闭";
    lastBrowserFailureSampleAt = 0;
    if (failureSamplingEnabled) clearFailureSamples();
    configureNativeDiagnostics();
  }

  function toggleExposureDiagnostics() {
    exposureDiagnosticsEnabled = developerModeEnabled && ui.exposureToggle.checked;
    exposureSamples = [];
    ui.exposureDetails.hidden = !exposureDiagnosticsEnabled;
    ui.exposureToggleLabel.textContent = exposureDiagnosticsEnabled ? "开启" : "关闭";
    renderExposureStats();
    configureNativeDiagnostics();
  }
  ...
  function resetDeveloperDiagnosticData() {
    durationDistribution = [];
    zxingErrorStats = createZxingErrorStats();
    qualityStats = createQualityStats();
    failureSamples = [];
    exposureSamples = [];
    lastDiagnosticRenderAt = 0;
    browserDiagnosticFrame = 0;
    lastBrowserFailureSampleAt = 0;
    renderDurationDistribution();
    renderZxingErrors();
  ...
  function recordDurationDistribution(value) {
    if (!durationDistributionEnabled) return;
    durationDistribution.push(finiteDuration(value));
    if (durationDistribution.length > DISTRIBUTION_SAMPLE_LIMIT) durationDistribution.shift();
    renderDeveloperDiagnosticsThrottled();
  }

  function renderDurationDistribution() {
    const count = durationDistribution.length;
    ui.distributionSamples.textContent = String(count);
    if (!count) {
      for (const element of [ui.distributionAverage, ui.distributionP50, ui.distributionP95, ui.distributionP99, ui.distributionMax]) element.textContent = "—";
      return;
    }
    const sorted = [...durationDistribution].sort((a, b) => a - b);
    const average = sorted.reduce((sum, value) => sum + value, 0) / count;
    ui.distributionAverage.textContent = formatMilliseconds(average);
    ui.distributionP50.textContent = formatMilliseconds(percentile(sorted, 0.50));
    ui.distributionP95.textContent = formatMilliseconds(percentile(sorted, 0.95));
    ui.distributionP99.textContent = formatMilliseconds(percentile(sorted, 0.99));
    ui.distributionMax.textContent = formatMilliseconds(sorted[count - 1]);
  }
  ...
  function recordZxingStatus(status) {
    if (!zxingErrorsEnabled) return;
    const key = Object.prototype.hasOwnProperty.call(zxingErrorStats, status) ? status : "exception";
    zxingErrorStats[key]++;
    renderDeveloperDiagnosticsThrottled();
  }

  function renderZxingErrors() {
    ui.zxingSuccess.textContent = String(zxingErrorStats.success);
    ui.zxingNoSymbol.textContent = String(zxingErrorStats.no_symbol);
    ui.zxingFormat.textContent = String(zxingErrorStats.format);
    ui.zxingChecksum.textContent = String(zxingErrorStats.checksum);
    ui.zxingUnsupported.textContent = String(zxingErrorStats.unsupported);
    ui.zxingException.textContent = String(zxingErrorStats.exception);
  }

  function recordQualitySample(metrics) {
    if (!imageQualityEnabled || !metrics || !Number.isFinite(metrics.brightness)) return;
    qualityStats.count++;
    for (const key of ["brightness", "contrast", "sharpness", "blackClip", "whiteClip"]) qualityStats[key] += finiteDuration(metrics[key]);
    renderDeveloperDiagnosticsThrottled();
  }

  function renderQualityStats() {
    const count = qualityStats.count;
    ui.qualitySamples.textContent = String(count);
    ui.qualityBrightness.textContent = count ? (qualityStats.brightness / count).toFixed(1) : "—";
    ui.qualityContrast.textContent = count ? (qualityStats.contrast / count).toFixed(1) : "—";
    ui.qualitySharpness.textContent = count ? (qualityStats.sharpness / count).toFixed(1) : "—";
    ui.qualityBlackClip.textContent = count ? `${(qualityStats.blackClip / count).toFixed(1)}%` : "—";
    ui.qualityWhiteClip.textContent = count ? `${(qualityStats.whiteClip / count).toFixed(1)}%` : "—";
  }

  function addFailureSample(dataUrl, frameNumber, reason) {
    if (!failureSamplingEnabled || !dataUrl) return;
    failureSamples.unshift({ dataUrl, frameNumber, reason, at: new Date().toLocaleTimeString() });
    if (failureSamples.length > FAILURE_SAMPLE_LIMIT) failureSamples.length = FAILURE_SAMPLE_LIMIT;
    renderFailureSamples();
  }

  function clearFailureSamples() {
    failureSamples = [];
    renderFailureSamples();
  }

  function renderFailureSamples() {
    ui.failureSamplesCount.textContent = `${failureSamples.length} / ${FAILURE_SAMPLE_LIMIT} 张`;
    ui.failureGallery.replaceChildren();
    if (!failureSamples.length) {
      const empty = document.createElement("p");
      empty.textContent = "尚无失败画面。";
      ui.failureGallery.appendChild(empty);
      return;
    }
    for (const sample of failureSamples) {
      const card = document.createElement("div");
      card.className = "failure-shot";
      const image = document.createElement("img");
      image.src = sample.dataUrl;
      image.alt = `失败帧 ${sample.frameNumber}`;
      const caption = document.createElement("span");
      caption.textContent = `#${sample.frameNumber} · ${sample.reason} · ${sample.at}`;
      card.append(image, caption);
      ui.failureGallery.appendChild(card);
    }
  }

  function recordExposureSample(metrics) {
    if (!exposureDiagnosticsEnabled || !metrics) return;
    const exposureTimeMs = Number(metrics.exposureTimeMs);
    if (!Number.isFinite(exposureTimeMs) || exposureTimeMs <= 0) return;
    exposureSamples.push({
      exposureTimeMs,
      iso: Number(metrics.iso),
      frameDurationMs: Number(metrics.sensorFrameDurationMs),
      fps: Number(metrics.sensorFps),
      ratio: Number(metrics.exposureRatio)
    });
    if (exposureSamples.length > EXPOSURE_SAMPLE_LIMIT) exposureSamples.shift();
    renderDeveloperDiagnosticsThrottled();
  }

  function renderExposureStats() {
    const count = exposureSamples.length;
    ui.exposureSamples.textContent = String(count);
    if (!count) {
      for (const element of [ui.exposureShutter, ui.exposureCurrent, ui.exposureIso,
        ui.exposureFrameDuration, ui.exposureFps, ui.exposureRatio, ui.exposureAverage,
        ui.exposureP50, ui.exposureP95, ui.exposureMin, ui.exposureMax]) element.textContent = "—";
      return;
    }
    const current = exposureSamples[count - 1];
    const sorted = exposureSamples.map(sample => sample.exposureTimeMs).sort((a, b) => a - b);
    const average = sorted.reduce((sum, value) => sum + value, 0) / count;
    ui.exposureShutter.textContent = formatShutter(current.exposureTimeMs);
    ui.exposureCurrent.textContent = formatMilliseconds(current.exposureTimeMs);
    ui.exposureIso.textContent = Number.isFinite(current.iso) && current.iso > 0 ? String(Math.round(current.iso)) : "—";
    ui.exposureFrameDuration.textContent = Number.isFinite(current.frameDurationMs) && current.frameDurationMs > 0
      ? formatMilliseconds(current.frameDurationMs) : "—";
    ui.exposureFps.textContent = Number.isFinite(current.fps) && current.fps > 0 ? `${current.fps.toFixed(1)} FPS` : "—";
    ui.exposureRatio.textContent = Number.isFinite(current.ratio) && current.ratio >= 0 ? `${current.ratio.toFixed(1)}%` : "—";
    ui.exposureAverage.textContent = formatMilliseconds(average);
    ui.exposureP50.textContent = formatMilliseconds(percentile(sorted, 0.50));
    ui.exposureP95.textContent = formatMilliseconds(percentile(sorted, 0.95));
    ui.exposureMin.textContent = formatMilliseconds(sorted[0]);
    ui.exposureMax.textContent = formatMilliseconds(sorted[count - 1]);
  }

  function formatShutter(exposureTimeMs) {
    if (!Number.isFinite(exposureTimeMs) || exposureTimeMs <= 0) return "—";
    if (exposureTimeMs >= 1000) return `${(exposureTimeMs / 1000).toFixed(2)} 秒`;
    const denominator = Math.max(1, Math.round(1000 / exposureTimeMs));
    return `1/${denominator} 秒`;
  }

  function renderDeveloperDiagnosticsThrottled() {
    const now = performance.now();
    if (now - lastDiagnosticRenderAt < TIMING_RENDER_INTERVAL) return;
    lastDiagnosticRenderAt = now;
    if (durationDistributionEnabled) renderDurationDistribution();
    if (zxingErrorsEnabled) renderZxingErrors();
    if (imageQualityEnabled) renderQualityStats();
    if (exposureDiagnosticsEnabled) renderExposureStats();
  }
  ...
  async function storeChunk(sessionId, index, bytes) {
    ...
    if (activeChunkBatch) {
      activeChunkBatch.push({ sessionId, index, copy, length: bytes.byteLength });
      return;
    }
  ...
  async function storeChunkBatch(rows) {
    if (!rows.length) return;
    if (memoryOnly || !database) {
      for (const row of rows) memoryChunks.set(row.index, row.copy);
      return;
    }
    await transactionPromise("readwrite", store => {
      for (const row of rows) {
        store.put({
          k: `${row.sessionId}:${row.index}`,
          sid: row.sessionId,
          i: row.index,
          data: row.copy,
          length: row.length
  ...
  async function deleteOtherSessions(sessionId) {
    if (memoryOnly || !database) return;
    await new Promise((resolve, reject) => {
      const transaction = database.transaction("chunks", "readwrite");
      ...
      request.onsuccess = () => {
        const cursor = request.result;
        if (!cursor) return;
        if (cursor.value.sid !== sessionId) cursor.delete();
        cursor.continue();
      };
      transaction.oncomplete = () => resolve();
      transaction.onerror = () => reject(transaction.error);
  ...
  async function clearStoredChunks() {
    memoryChunks.clear();
    if (memoryOnly || !database) return;
    await transactionPromise("readwrite", store => store.clear());
  }
})();
